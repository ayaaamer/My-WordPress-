...
if ( ! class_exists( 'WPTT_WebFont_Loader' ) ) {
	...
	class WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.1.0
		 * @param string $aster_storefront_url The remote URL.
		 */
		public function __construct( $aster_storefront_url = '' ) {
			$this->remote_url = $aster_storefront_url;
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $font_family => $files ) {
				...
				foreach ( $files as $aster_storefront_url ) {

					// Get the filename.
					$filename  = basename( wp_parse_url( $aster_storefront_url, PHP_URL_PATH ) );
					...
					$tmp_path = download_url( $aster_storefront_url );
			...
			if ( $change ) {
				...
				foreach ( $stored as $aster_storefront_url => $path ) {
					if ( ! file_exists( $path ) ) {
						unset( $stored[ $aster_storefront_url ] );
					}
				}
		 ...
		 * @access public
		 * @since 1.0.0
		 * @param string $aster_storefront_format The format to be used. Use "woff" or "woff2".
		 * @return void
		 */
		public function set_font_format( $aster_storefront_format = 'woff2' ) {
			$this->font_format = $aster_storefront_format;
		}
		 ...
		 * @access protected
		 *
		 * @param string $aster_storefront_url The URL.
		 *
		 * @return string
		 */
		protected function get_absolute_path( $aster_storefront_url ) {

			// If dealing with a root-relative URL.
			if ( 0 === stripos( $aster_storefront_url, '/' ) ) {
				$aster_storefront_parsed_url = parse_url( $this->remote_url );
				return $aster_storefront_parsed_url['scheme'] . '://' . $aster_storefront_parsed_url['hostname'] . $aster_storefront_url;
			}

			return $aster_storefront_url;
		}
	}
}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	 ...
	 * @since 1.0.0
	 *
	 * @param string $aster_storefront_url    The URL of the remote webfont.
	 * @param string $aster_storefront_format The font-format. If you need to support IE, change this to "woff".
	 *
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_styles( $aster_storefront_url, $aster_storefront_format = 'woff2' ) {
		$aster_storefront_font = new WPTT_WebFont_Loader( $aster_storefront_url );
		$aster_storefront_font->set_font_format( $aster_storefront_format );
		return $aster_storefront_font->get_styles();
	}
}
...
if ( ! function_exists( 'wptt_get_webfont_url' ) ) {
	 ...
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_url( $aster_storefront_url, $aster_storefront_format = 'woff2' ) {
		$aster_storefront_font = new WPTT_WebFont_Loader( $aster_storefront_url );
		$aster_storefront_font->set_font_format( $aster_storefront_format );
		return $aster_storefront_font->get_url();
	}
}